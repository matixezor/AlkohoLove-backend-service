...
class FollowersDatabaseHandler:
    ...
    async def delete_user_from_followers(collection: Collection[Followers], user_id: str,
        ...
        collection.update_one({'_id': ObjectId(user_id)}, {'$pull': {'followers': follower_user_id}})

    @staticmethod
    async def add_user_to_followers(collection: Collection[Followers], user_id: str,
                                    follower_user_id: ObjectId) -> None:
        collection.update_one({'_id': ObjectId(user_id)}, {'$push': {'followers': follower_user_id}})

    @staticmethod
    async def check_if_user_in_followers(collection: Collection[Followers], user_id: str,
                                         follower_user_id: ObjectId) -> bool:
        if collection.find_one({'_id': ObjectId(user_id), 'followers': follower_user_id}):
            return True
        else:
            return False
